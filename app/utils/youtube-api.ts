...
class YouTubeApiKeyManager {
  ...
  constructor() {
    ...
    ].filter(config => Boolean(config.key)) as { key: string; name: string }[];
  ...
  getCurrentKey(): string | null {
    ...
    if (this.apiKeys[this.currentKeyIndex].status === ApiKeyStatus.ACTIVE) {
      ...
      console.debug(`[YouTubeAPI] Using API key from ${this.apiKeys[this.currentKeyIndex].envName}`);
      return this.apiKeys[this.currentKeyIndex].key;
    }
    
    // Try to find an active key
    const activeKeyIndex = this.apiKeys.findIndex(key => key.status === ApiKeyStatus.ACTIVE);
    if (activeKeyIndex >= 0) {
      this.currentKeyIndex = activeKeyIndex;
      this.apiKeys[this.currentKeyIndex].lastUsed = new Date();
      console.debug(`[YouTubeAPI] Switched to API key from ${this.apiKeys[this.currentKeyIndex].envName}`);
      return this.apiKeys[this.currentKeyIndex].key;
    }
  ...
  rotateToNextKey(): string | null {
    ...
    while (checkedKeys < this.apiKeys.length) {
      ...
      if (this.apiKeys[this.currentKeyIndex].status === ApiKeyStatus.ACTIVE) {
        this.apiKeys[this.currentKeyIndex].lastUsed = new Date();
        console.debug(`[YouTubeAPI] Rotated to API key from ${this.apiKeys[this.currentKeyIndex].envName}`);
        return this.apiKeys[this.currentKeyIndex].key;
      }
      checkedKeys++;
    }
  ...
  markCurrentKeyQuotaExceeded(errorMessage?: string): void {
    ...
    console.warn(`[YouTubeAPI] Quota exceeded for API key from ${this.apiKeys[this.currentKeyIndex].envName}`);
  }
  ...
  markCurrentKeyError(errorMessage?: string): void {
    ...
    if (this.apiKeys[this.currentKeyIndex].errorCount >= 3) {
      ...
      console.error(`[YouTubeAPI] API key from ${this.apiKeys[this.currentKeyIndex].envName} marked as error after ${this.apiKeys[this.currentKeyIndex].errorCount} errors`);
    }
  }
   ...
   */
  getKeyStatuses(): { index: number; envName: string; status: ApiKeyStatus; lastUsed: Date; errorCount: number }[] {
 ...
 */
export function detectYouTubeApiErrorType(response: Response, responseData: any): YouTubeApiErrorType {
 ...
 */
export function handleYouTubeApiError(response: Response, responseData: any): boolean {
...
export async function fetchYouTubeApi<T>(
  ...
  while (retries <= retryConfig.maxRetries) {
    try {
      ...
      const currentKey = keyStatuses.find(k => k.status === ApiKeyStatus.ACTIVE);
      
      // Add the API key to the URL
      const separator = url.includes('?') ? '&' : '?';
      const apiUrl = `${url}${separator}key=${apiKey}`;
      
      console.debug(`[YouTubeAPI] Making request with key from ${currentKey?.envName || 'unknown'}`);
